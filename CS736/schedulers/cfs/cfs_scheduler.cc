...
#define ALPS_SERVER_IP "127.0.0.1"
#define ALPS_SERVER_PORT 8083
#define ALPS_NUM_CLASSES 100
...
CfsScheduler::CfsScheduler(Enclave* enclave, CpuList cpulist,
  ...
  // --- ADD THIS: Initialize Policies and Start Thread ---
  
  // Optional: Initialize policies to safe defaults to prevent reading garbage 
  // before the first network packet arrives.
  for (auto& policy : alps_policies_) {
      policy = {0, 10000000}; // Default: Prio 0, 10ms timeslice (10,000,000 ns)
  }

  // Start the background thread that talks to the Python server.
  // We pass 'this' so the thread can access the 'alps_policies_' member.
  policy_thread_ = std::thread(&CfsScheduler::PolicyUpdateLoop, this);
}
...
void CfsScheduler::PolicyUpdateLoop() {
    int sock = -1;
    ...
    while (!stop_policy_thread_) {
        ...
        if (sock < 0) {
            ...
            server_addr.sin_port = htons(8083); // Hardcoded ALPS port
            ...
            fprintf(stderr, "ALPS: Connected to Python Server!\n");
        }

        // --- 2. Prepare Stats (Closed Loop) ---
        {
        ...
        // --- 3. Send Stats to Python ---
        if (send(sock, stats_buffer, sizeof(stats_buffer), 0) < 0) {
            perror("ALPS Send failed");
            close(sock);
            sock = -1;
            continue;
        }

        // --- 4. Receive New Policy from Python ---
        uint64_t recv_buffer[100];
        int total_bytes = 0;
        int target_bytes = sizeof(recv_buffer); // 800 bytes
        bool recv_failed = false;

        while (total_bytes < target_bytes) {
             int n = recv(sock, (char*)recv_buffer + total_bytes, 
                          target_bytes - total_bytes, 0);
             if (n <= 0) {
                 recv_failed = true;
                 break;
             }
        ...
        if (recv_failed) {
            fprintf(stderr, "ALPS: Recv failed or disconnected\n");
            close(sock);
            sock = -1;
            continue;
        }
        ...
        // --- 5. Apply New Policy ---
        {
            absl::MutexLock lock(&policy_mu_);
            for (int i = 0; i < 100; ++i) {
                int class_id = i + 1;
                
                uint64_t val = recv_buffer[i];
                int priority = val % 1000000;
                uint64_t ts_part = val - priority;
                
                if (ts_part == 0) ts_part = 10000000; // 10ms default
                
                alps_policies_[class_id].priority = priority;
                alps_policies_[class_id].timeslice_ns = ts_part;
            }
        }

        // --- 6. IMPROVED LOGGING ---
        // We iterate through classes 1-4 specifically for your test workload.
        // This shows the Closed Loop: Sent Wait Time -> Received Policy.
        fprintf(stderr, "ALPS CYCLE [Sent Stats -> Recv Policy]:\n");
        for (int c = 1; c <= 4; ++c) {
             // stats_buffer is 0-indexed, so Class 1 is at index 0
             uint64_t avg_wait_ns = stats_buffer[c-1];
             
             // Convert to milliseconds for easier reading
             double wait_ms = avg_wait_ns / 1000000.0;
             double ts_ms = alps_policies_[c].timeslice_ns / 1000000.0;

             fprintf(stderr, "  Class %d: Wait %.2f ms -> Prio %d, TS %.0f ms\n",
                     c, wait_ms, alps_policies_[c].priority, ts_ms);
        }
        fprintf(stderr, "---------------------------------------------------\n");
