...
struct CfsTask : public Task<> {
  ...
  // CORRECTED: Pure comparison. No file I/O here. 
  // File I/O happens in CfsSchedule, which sets 'seal_prio'.
  static bool Less(CfsTask* a, CfsTask* b) {
    if (a->seal_prio != b->seal_prio) {
      // Higher priority value comes first (or last depending on your logic).
      // Assuming smaller integer = higher priority (like nice values):
      return a->seal_prio < b->seal_prio; 
    }
    
    // Tie-breaker: vruntime
  ...
  // --- CUSTOM FIELDS FOR ALPS ---
  uint64_t runtime_at_current_time_slice = 0;
  
  // Stores the current dynamic priority from the Python server (updated in CfsSchedule)
  mutable int seal_prio = 0; 

  // Stores the static Class ID (1-100) read from /tidinfo/ file.
  // We cache it here so we only read the file once per task.
  int class_id = 0; 
  // ------------------------------
  ...
  // [ALPS] Track when the task entered the RQ to calculate wait time later
  absl::Time enqueue_time = absl::Now();
};
...
class CfsScheduler : public BasicDispatchScheduler<CfsTask> {
 ...
 private:
  std::array<AlpsPolicy, 101> alps_policies_; // 1-100 indexed
  ...
  absl::Mutex policy_mu_;
  absl::Mutex stats_mu_; 
  
  std::thread policy_thread_;
  std::atomic<bool> stop_policy_thread_{false};

  void PolicyUpdateLoop();
